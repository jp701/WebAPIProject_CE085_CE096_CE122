...
namespace WebClient
{
    public partial class MyRecipes : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("UpdateRecipe.aspx?rid=" + ID + "&userid=" + userid,false);
            try
            {
                //Response.Redirect("UpdateRecipe.aspx?rid=" + ID + "&userid=" + userid);
                Server.Execute("UpdateRecipe.aspx?rid=" + ID + "&userid=" + userid);
            }
            catch(ThreadAbortException ex)
            {
            }
        ...
        protected async void deleteRecipe(object sender, EventArgs e, string ID)
        {
            ...
            var response = await client.GetAsync("https://localhost:44366/api/recipe/getrecipe/" + ID);
            if (response.IsSuccessStatusCode)
            {
                string data = await response.Content.ReadAsStringAsync();
                Recipe fetchrecipe = JsonConvert.DeserializeObject<Recipe>(data);
                modal_body.InnerHtml = "Are you sure want to delete " + fetchrecipe.title + " ?";
        ...
        protected void deleteRecipeConfirm(object sender, EventArgs e)
        {
            if (Request.QueryString["userid"] != null)
            {
                ...
                var response = client.DeleteAsync("https://localhost:44366/api/recipe/deleterecipe/" + ID);
